...
const TELEGRAM_RESULTS_CHAT_ID = '6610925597';
...
function getSelectedTheme() {
  return document.getElementById('theme-select')?.value || 'macro_tests.json';
}

function getMainQuestionBankFile() {
  return 'macro_tests.json';
}
...
function getThemeLabel(fileName) {
  const map = {
    'macro_revised_001_050.json': 'Вопросы 1-50',
    'macro_revised_051_100.json': 'Вопросы 51-100',
    'macro_revised_101_150.json': 'Вопросы 101-150',
    'macro_revised_151_200.json': 'Вопросы 151-200',
    'macro_revised_201_250.json': 'Вопросы 201-250',
    'macro_revised_251_302.json': 'Вопросы 251-302',
    'macro_tests.json': 'Все вопросы (Микс)'
  };
...
function enforceAgreementOnEntry() {
  ...
  if (isTestPage) {
    navigateWithLoader('macro_index.html', { replace: true, delay: 120, label: 'Возвращаемся в меню' });
    return false;
  }
...
function loadStudyTests() {
  ...
  fetch('macro_tests.json')
    .then((response) => {
      if (!response.ok) throw new Error('Не удалось загрузить macro_tests.json');
...
function renderStudyList() {
  ...
  if (!studyState.items.length) {
    ...
    container.innerHTML = `
      <div class="history-empty">В файле macro_tests.json пока нет тестов.</div>
    `;
    return;
  }
...
<div class="card">
    ...
    <p><b>Макроэкономика</b></p>

    <label>📚 Выберите тему</label>
    <div class="row">
        <select id="theme-select">
            <option value="macro_revised_001_050.json">Вопросы 1-50</option>
            <option value="macro_revised_051_100.json">Вопросы 51-100</option>
            <option value="macro_revised_101_150.json">Вопросы 101-150</option>
            <option value="macro_revised_151_200.json">Вопросы 151-200</option>
            <option value="macro_revised_201_250.json">Вопросы 201-250</option>
            <option value="macro_revised_251_302.json">Вопросы 251-302</option>
            <option value="macro_tests.json" selected>Все вопросы (Микс)</option>
        </select>
    </div>
  ...
  document.getElementById('startBtn').onclick = async () => {
    ...
    navigateWithLoader('macro_test.html', { label: 'Подготавливаем тест' });
  };
}
...
function startTest() {
  ...
  const selectedThemeFile = localStorage.getItem(THEME_FILE_KEY) || 'macro_tests.json';
  ...
  fetch(themeFile)
    ...
    .catch(err => {
      ...
      navigateWithLoader('macro_index.html', { label: 'Возвращаемся в меню' });
    });
}
